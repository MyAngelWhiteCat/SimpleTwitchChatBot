...
CapRequest, обработка Ping-Pong, Join/Part каналов. Поддерживает множетсвенное чтение нескольких каналов сразу.

Чат бот передается в виде шаблона, интерфейс которого должен иметь метод ParseAndExecute() принимающее класс Message.
...
Клиент довольно гибкий, и сам по себе уже является неплохой реализацией, но без чат бота он бесполезен. Именно по этому чат бот передается в виде шаблона. Нет бота - нет и клиента.
